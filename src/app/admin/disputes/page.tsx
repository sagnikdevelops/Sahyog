  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="border-b border-[#E5E7EB] pb-4">
        <h1 className="text-2xl font-bold text-[#142D52]">Dispute Resolution Center</h1>
      ...
      <div className="space-y-4">
        {disputes.length === 0 ? (
          <div className="p-8 text-center bg-white rounded-lg border border-[#E5E7EB] text-xs text-[#6B7280]">
            No disputes on record. Customer satisfaction is 100% in good standing.
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-4">
            {disputes.map((d) => (
              <Card key={d.id} className="border-[#E5E7EB] bg-white">
                <CardContent className="p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 text-xs">
                  <div className="space-y-1.5 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-[#142D52]">Dispute on #{d.bookingNumber}</span>
                    ...
                    <p className="text-[#4B5563] leading-relaxed">{d.description}</p>
                    <p className="text-[10px] text-[#6B7280]">
                      Filed by {d.raisedByName} on {formatDate(d.createdAt)}
                    </p>

                    {d.status === "RESOLVED" && (
                      <div className="p-3 bg-[#F9FAF7] rounded border border-[#E5E7EB] mt-2 space-y-0.5">
                        <p className="font-bold text-[11px] text-[#047857]">Resolution: {d.resolutionAction}</p>
                        <p className="text-[#4B5563] text-[11px]">{d.adminNotes}</p>
                        <p className="text-[10px] text-[#6B7280]">Resolved by: {d.resolvedBy} on {formatDate(d.resolvedAt || d.createdAt)}</p>
                      </div>
                    )}
                  </div>
                  ...
                  {d.status !== "RESOLVED" && (
                    <Button
                      ...
                      className="text-xs bg-[#047857] text-white hover:bg-[#065F46] shrink-0"
                    >
                      Investigate &amp; Resolve
                    </Button>
                  )}
      ...
      <Dialog open={Boolean(selectedDispute)} onOpenChange={() => setSelectedDispute(null)}>
        <DialogContent className="max-w-md bg-white border border-[#E5E7EB]">
          <DialogHeader>
            <DialogTitle className="text-base font-bold text-[#142D52]">Resolve Dispute</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-2 text-xs">
            <div className="p-3 bg-[#F9FAF7] rounded border border-[#E5E7EB]">
              <p className="font-bold text-[#142D52]">Booking #{selectedDispute?.bookingNumber}</p>
              <p className="text-[#DC2626] font-semibold mt-0.5">{selectedDispute?.reason}</p>
              <p className="text-[#4B5563] mt-1">{selectedDispute?.description}</p>
            </div>

            <div>
              <label className="font-bold text-[#142D52] block mb-1">Resolution Action</label>
              <select
                value={actionType}
                onChange={(e) => setActionType(e.target.value as any)}
                className="w-full h-9 rounded-md border border-[#E5E7EB] bg-white px-3 text-xs text-[#142D52]"
              >
            ...
            <div>
              <label className="font-semibold block mb-1 text-[#142D52]">Cooperative Committee Notes *</label>
              <Textarea
                rows={3}
                value={adminNotes}
                onChange={(e) => {
          ...
          <DialogFooter>
            <Button variant="outline" size="sm" onClick={() => setSelectedDispute(null)} className="border-[#E5E7EB] text-[#142D52]">
              Cancel
            </Button>
            <Button onClick={handleResolve} size="sm" className="bg-[#047857] hover:bg-[#065F46] text-white">
              Confirm Resolution
            </Button>
          </DialogFooter>
        </DialogContent>
